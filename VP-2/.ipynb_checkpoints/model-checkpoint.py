# =============================================================================
# VirulentPred 2.0 最终完美版 —— 每一折都输出全部 8 个指标
...
def load_fasta(path):
    ...
    if seq:
        seqs.append(seq)
    print(f"✅ 读取序列：{len(seqs)} | 正：{sum(labels)} | 负：{len(labels)-sum(labels)}")
...
def run_kfold(seqs, labels):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        print(f"{Sn:.4f}\t{Sp:.4f}\t{Acc:.4f}\t{MCC:.4f}\t{Precision:.4f}\t{F1:.4f}\t{ROC_AUC:.4f}\t{PR_AUC:.4f}")

    # 输出最终平均
    mean = np.mean(results, axis=0)
    print("\n" + "="*70)
    print(" VirulentPred 2.0 10折平均结果 ")
    print("="*70)
    print("Sn\tSp\tAcc\tMCC\tPrec\tF1\tROC-AUC\tPR-AUC")
    print(f"{mean[0]:.4f}\t{mean[1]:.4f}\t{mean[2]:.4f}\t{mean[3]:.4f}\t{mean[4]:.4f}\t{mean[5]:.4f}\t{mean[6]:.4f}\t{mean[7]:.4f}")
